...
def test_find_ccusage_from_path(tmp_path):
    ...
    fake.chmod(0o755)
    from ai_cost_compare.providers.claude.fetch import fetch_daily

    with patch("ai_cost_compare.providers.claude.fetch.find_ccusage", return_value=fake):
...
def test_find_ccusage_missing():
    ...
    from ai_cost_compare.providers.claude.fetch import find_ccusage

    with patch("ai_cost_compare.providers.claude.fetch._nvm_ccusage_paths", return_value=[]):
